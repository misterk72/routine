...
The app sync layer uses its own tables:
- `users`
- `health_entries`
- `workout_entries`

`workout_entries` mirrors a subset of `workouts` plus computed fields:
- `start_time`, `duration_minutes`, `distance_km`
- `avg_speed_kmh`, `calories`, `calories_per_km`
- `avg_heart_rate`, `min_heart_rate`, `max_heart_rate`
- `sleep_heart_rate_avg`, `vo2_max`
- `program`, `notes`
- `client_id`, `deleted`, `last_modified`
...
- `import_manual_workouts_mariadb.py`
  ...
  - Example:
    ...
    python docker-sqlite/import_manual_workouts_mariadb.py \
      ...
      --user-profile-id 1 --source-id 4 --ignore-duplicates --apply
    ```
